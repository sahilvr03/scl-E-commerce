  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        toast.error(error.message || 'Failed to load For You products', {
          style: {
            background: '#FFFFFF',
            color: '#1F2937',
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 max-w-7xl">
        ...
        >
          <div className="flex items-center justify-between">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 flex items-center">
              <ShoppingBag className="w-8 h-8 mr-2 text-orange-500" />
              For You
            </h1>
            <motion.button
        ...
        {products.length === 0 ? (
          ...
          >
            ...
            <p className="text-lg text-gray-600 mb-6">No products available in this section.</p>
            <Link href="/products">
              <motion.button
          ...
          >
            {products.map((product) => (
              <motion.div key={product._id} variants={itemVariants}>
                <ProductCard product={product} isSale={product.type === 'flashSale'} />
              </motion.div>
            ))}
          </motion.div>
        )}

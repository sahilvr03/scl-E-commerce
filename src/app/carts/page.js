      ...
      try {
        ...
        if (response.ok) {
          ...
          if (validItems.length < data.items.length) {
            console.warn("Some invalid cart items removed");
            toast.error("Some cart items were invalid and removed.");
          }
    ...
    try {
      ...
      if (response.ok) {
        ...
        window.dispatchEvent(
          new CustomEvent("cartUpdated", { detail: { count: validItems.length } })
        );
      } else {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <ShoppingCart className="w-12 h-12 text-indigo-500 animate-spin" />
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={() => router.push("/pages/ForYouPage")}
